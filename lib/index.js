      ...
      s3 = new AWS.S3();
		...
		function writeList(param, next) {
			async.each(Object.keys(files), function(file, callback) {
				...
				if(config['gzip'] && (new RegExp(config['gzip'])).test(file)) {
					param.ContentEncoding = 'gzip';
				} else {
					param.ContentEncoding = null;
				}
				if(config['max-age']) {
					param.CacheControl = 'public, max-age=' + config['max-age'];
				} else {
					param.CacheControl = null;
				}
				debug("writing file: ", file, "with contentType = ", param.ContentType, "with contentEncoding = ", param.ContentEncoding, "with cacheControl = ", param.CacheControl);
